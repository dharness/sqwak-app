...
class MlAppsPage extends Component {
  ...
  renderAppPreviewCards() {
    return this.props.mlApps.map((mlApp, i) => {
      const numClasses = mlApp.mlClasses ? mlApp.mlClasses.length : 0;
      return (
        <AppPreviewCard
          onCardSelected={() => {
            this.openApp(mlApp.id);
          }}
          key={mlApp.id}
          appId={mlApp.id}
          name={mlApp.app_name}
          ...
          onDeleteClicked={(id) => {
            this.showConfirmDeleteWarning(mlApp.app_name, id);
          }}
        />
      );
    });
  ...
  render() {
    let shouldHideBgImg =
      this.props.mlApps.length > 0 || this.props.isFetchingApps;
    return (
      <div
        className="sq-apps-page"
        style={{ opacity: this.props.isFetchingApps ? 0 : 1 }}
      >
        ...
        <div className="sq-apps-page--content">
          <div className="sq-apps-page--header">
            ...
            <PlushButton
              ...
              onClick={this.showCreateAppModal.bind(this)}
            />
          </div>
          ...
          <div
            className={
              "sq-apps-page--bg-img" + (shouldHideBgImg ? " hiddden" : "")
            }
            style={{
